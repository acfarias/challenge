...
{
    public class GetCensusPaginatedQueryHandler : IRequestHandler<GetCensusPaginatedQuery, CensusPaginatedResponseDto>
    {
        private readonly ICensusRepository _censusRepository;

        public GetCensusPaginatedQueryHandler(ICensusRepository censusRepository)
        {
        ...
        public async Task<CensusPaginatedResponseDto> Handle(GetCensusPaginatedQuery request, CancellationToken cancellationToken)
        {
            ...
            var dataResult = await _censusRepository.GetCensusPaginated(request.Page, request.ItemsPerPage, request.SearchClause, cancellationToken);
